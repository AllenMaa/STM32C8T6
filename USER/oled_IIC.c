...
//  Sample usage:			
//-------------------------------------------------------------------------------------------------------------------
void OLED_Print_Num(uint8_t x, uint8_t y, uint8_t num)
{
	uint8_t *ch1,ch[7];
...
{
	uint8_t *ch1,ch[7];
	if(num<0)   {num = -num;OLED_ShowStr(x, y, (uint8_t *)"-",1);}
	else         OLED_ShowStr(x, y, (uint8_t *)" ",1);
	x+=6;
	OLED_HEXACSII(num,ch);
	ch1 =&ch[1];
	while(*ch1 != '\0')
